...
#else
  SPISettings _fastSPI = SPISettings(8000000, MSBFIRST, SPI_MODE0);
#endif
...
void port_set_spi_hz(uint32_t hz) {
  _fastSPI = SPISettings(hz, MSBFIRST, SPI_MODE0);
                ...
                uint16_t readLength, uint8_t *readBuffer)
{
  ...
  SPI.beginTransaction(*_currentSPI);
  digitalWrite(_ss, LOW);
  SPI.writeBytes(headerBuffer, headerLength);
  SPI.transferBytes(Buf, readBuffer, readLength);
  digitalWrite(_ss, HIGH);
  SPI.endTransaction();
  return 0;
}
               ...
               uint16_t bodyLength, uint8_t *bodyBuffer)
{
  ...
  digitalWrite(_ss, LOW);
  SPI.writeBytes(headerBuffer, headerLength);
  SPI.writeBytes(bodyBuffer, bodyLength);
  digitalWrite(_ss, HIGH);
  SPI.endTransaction();
  return 0;
}
